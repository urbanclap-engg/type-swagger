...
export default class SchemaConvertor {
    ...
    public addFunctionDefinition(apiName: string, requestRefName: string, responseRefName: string): void {
        const processor = this.processor;
        processor.output(`${apiName}(input: ${requestRefName},  isAsync?: string): Promise<${responseRefName}>;`);
        processor.outputLine();
    }
    public getFunctionDeclarationObj(apiName: string, requestRefName: string, responseRefName: string): string {
        return `${apiName}(input: ${requestRefName},  isAsync?: string): Promise<${responseRefName}>;`
    }
    public outputNestedFunctions(obj: object){
        Object.keys(obj).forEach((key) =>{
            ...
            Object.keys(childObj).forEach(prop => {
                if (typeof childObj[prop] === 'object') {
                    this.outputNestedFunctions({ [prop]: childObj[prop] });
                } else {
                    processor.output(`${childObj[prop]}`);
                    processor.outputLine();
                }
            });
